...
class MovieCommonService:
    ...
    async def get_sorted_list(
        ...
        key = f'{es_index}:{sort_field}:{sort_type}:{genre_id}:{page_number}:{page_size}'
        data_list = await self.cache.get_list(key=key, model=model)
        if not data_list:
            data_list = await self.database.get_list(page_number, page_size, es_index, model, query)
            await self.cache.put_list(key=key, data_list=data_list, cache_timeout=cache_timeout)
        return data_list
    ...
    async def get_similar_list(
        ...
        data_list = await self.cache.get_list(key=key, model=model)
        if not data_list:
            data_list = await self._fetch_similar_movies_by_genres(movie_id, es_index, model, cache_timeout)
            await self.cache.put_list(key=key, data_list=data_list, cache_timeout=cache_timeout)
        return data_list
