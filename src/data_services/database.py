...
class Database(ABC):
    @abstractmethod
    async def get_by_id(self, id: UUID, model: BaseModel, es_index: str) -> BaseModel | None:
        pass

    @abstractmethod
    async def search(self, search_string: str, search_field: str, page_number: int, page_size: int, es_index: str,
                     model: BaseModel) -> list[BaseModel]:
        pass

    @abstractmethod
    async def get_list(self, page_number: int, page_size: int, es_index: str, model: BaseModel, query: dict = None) -> \
            list[BaseModel]:
        pass
